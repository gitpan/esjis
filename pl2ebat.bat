...
Usage:  $0 [-h]
   ...
   or:  $0 [-w] [-u] [-n ntargs] [-o otherargs] [-s stripsuffix] [files]
        ...
        -a argstring    arguments to invoke perl with in generated file
                            ...
                            with previous pl2ebat versions).
        -u              update files that may have already been processed
                            by (some version of) pl2ebat.
...
sub process {
 my( $file )= @_;
    ...
    open( FILE, "perl esjis.pl $file | " ) or die "$0: Can't open $file: $!";
    @file = <FILE>;
    foreach $line ( @file ) {
	$linenum++;
	if ( $line =~ /^:endofperl\b/ ) {
...
B<pl2ebat> [B<-w>] S<[B<-a> I<argstring>]> S<[B<-s> I<stripsuffix>]> [files]

B<pl2ebat> [B<-w>] S<[B<-n> I<ntargs>]> S<[B<-o> I<otherargs>]> S<[B<-s> I<stripsuffix>]> [files]
...
If the script appears to have already been processed by B<pl2ebat>,
...
=head1 EXAMPLES
	...
	C:\> pl2ebat foo.pl bar.PM 
	[..creates foo.bat, bar.PM.bat..]
	
	C:\> pl2ebat -s "/\.pl|\.pm/" foo.pl bar.PM
	[..creates foo.bat, bar.bat..]
	
	C:\> pl2ebat < somefile > another.bat
	
	C:\> pl2ebat > another.bat
